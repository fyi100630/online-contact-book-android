    ...
    override fun ContentDrawScope.draw() {
        ...
        if (shadowLayer != null) {
            try {
                ...
                val widthInt = ceil(size.width + radius * 4f + offsetX).toInt()
                val heightInt = ceil(size.height + radius * 4f + offsetY).toInt()

                if (widthInt > 0 && heightInt > 0) {
                    val shadowSize = IntSize(widthInt, heightInt)
                ...
                // Safeguard against shadow recording errors
            }
        }
